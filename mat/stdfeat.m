...
%mean absolute jerk
if get; f=vertcat(f,getMAjerk(x,sf)); end 
name=vertcat(name,{'Mean Absolute Jerk'});

%spectral arc length (measure of smoothness)
if get; f=vertcat(f,SpectralArcLength(x,(1/sf))); end %note: spectral arc length function uses T instead of f, addl inputs for cutoff frequencies unused
name = vertcat(name,{'Spectral Arc Length'});
end